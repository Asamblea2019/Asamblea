...
<%@page import="TemasDAO.PreguntaDAO"%>
<%@page import="TemasVO.PreguntaVO"%>
<%@page import="TemasVO.PreguntaVO"%>
<%@page import="TemasDAO.VotosDAO"%>
<%@page import="TemasVO.VotosVO"%>
<%@page import="java.util.ArrayList"%>
<%@page import="TemasDAO.TemasDAO"%>
        ...
        <section class="banner_part">
            ...
            <div class="container" style="width: 45%">
                <div id="Contenedor-form">  
                    ...
                    <%
                        TemasVO temVO = new TemasVO();
                        TemasDAO temDAO = new TemasDAO();

                        VotosVO votVO = new VotosVO();
                        VotosDAO votDAO = new VotosDAO();

                        PreguntaVO preVO = new PreguntaVO();
                        PreguntaVO preVOAD = new PreguntaVO();
                        PreguntaDAO preDAO = new PreguntaDAO();

                        ArrayList<TemasVO> listatemas = temDAO.select();
                        ArrayList<PreguntaVO> listapreguntasCn = preDAO.selectCons();
                        ArrayList<PreguntaVO> listapreguntasAd = preDAO.selectAdmi();
                        ArrayList<VotosVO> listavotos = votDAO.listar();

                        for (int i = 0; i < listatemas.size(); i++) {
                            temVO = listatemas.get(i);
                            
%>
                    <h3><%= temVO.getTema()%></h3>
                    

                    <% for (int j = 0; j < listapreguntasCn.size(); j++) {
                            preVO = listapreguntasCn.get(j);
                            votVO = listavotos.get(i);
                            if (preVO.getIdf_votacion().equals(votVO.getId_Votacion())) {
                            //if (temVO.getId_Tema().equals(votVO.getIdf_Tema())) {        
                                
                    %>
                    <form action="Respuesta" method="post">
                        <h6><%= preVO.getPregunta()%></h6>
                        
                        <input type="submit" name="resupesta" value="Si" >
                        <input type="submit" name="resupesta" value="NO" >
                        <input type="hidden" name="id_pregunta" value="<%= preVO.getId_pregunta()%>" >
...
<a href="consultar_respuesta.jsp" class="btn btn-outline-primary">Consultar</a>
